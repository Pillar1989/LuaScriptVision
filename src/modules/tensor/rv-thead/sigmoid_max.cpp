/*
 * T-Head RISC-V optimized sigmoid + max fusion (row-major scan + vectorized sigmoid)
 * Uses RVV (RISC-V Vector Extension) for C906/C908 processors
 *
 * Optimizations:
 * 1. Sigmoid monotonicity: max(sigmoid(x)) = sigmoid(max(x))
 * 2. Row-major traversal: outer loop over classes, inner vectorized over anchors
 *    - Changes from column-major (stride access) to row-major (sequential access)
 *    - Fixes cache thrashing issue with stride=6400 (25KB)
 * 3. Vectorized sigmoid: uses rv_thead::exp_ps() for fast exp computation
 */
...
LuaIntf::LuaRef Tensor::sigmoid_max_with_argmax_thead(lua_State* L, int axis) const {
    check_cpu();

    if (ndim() != 2 || axis != 0) {
        throw std::runtime_error(
            "sigmoid_max_with_argmax_thead: requires 2D tensor with axis=0");
    }
    ...
    // Step 1 & 2: Use shared RVV-optimized max/argmax computation
    rv_thead::compute_max_and_argmax(
        input_data, num_classes, n_anchors,
        max_values.data(), max_indices.data());

    // Step 3: Apply vectorized sigmoid using shared function
    rv_thead::apply_vectorized_sigmoid(max_values.data(), n_anchors);
